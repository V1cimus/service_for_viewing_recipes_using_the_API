...
class BaseIngredientsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if name:
            queryset = (
                queryset.filter(name__icontains=name.lower()).annotate(
                    order=Case(When(name__istartswith=name, then=1))
                ).order_by('order', 'name')
            )
        return queryset
...
class AuthorSubscriptionViewSet(CreateAndDectroyViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        SubscribAuthor.objects.create(user=subscriber, author=subscribing,)
        serializer = AuthorSubSerializer(
            context={'request': request}, instance=subscribing,
        )
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.request.method == 'GET':
            return (IsAuthenticatedOrReadOnly(), IsNotBanPermission(),)
        if self.request.method == 'PATCH':
            return (AuthorPermission(), IsNotBanPermission(),)
